...
//3. new File(String parent, String child)
		...
		System.out.println("절대 경로 : " + file4.getAbsolutePath()); // 내가 어디에 있는 상관없음, D나 C드라이브로 시작 
		System.out.println("경로 : " + file4.getPath()); // 생성자 경로 그대로 가져옴. 프로젝트 내에 있는 이미지를 참고
		System.out.println("표준 경로 : " + file4.getCanonicalPath()); //.이 없음. 필요한 것을 최종 계산 한 것 
		System.out.println("-----------------------------------");
		
/* 디렉토리(폴더) 만들기
	1. mkdir() => File객체의 경로 중 마지막 위치의 디렉토리를 만든다. (make directory)
				* 중간 경로 모두 미리 만들어져 있어야 한다.
		              
	2. mkdirs() => 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 만든다. */
		                                  
// => 1, 2번 모두 폴더 만들기를 성공하면 true, 실패하면 false를 반환한다. 
