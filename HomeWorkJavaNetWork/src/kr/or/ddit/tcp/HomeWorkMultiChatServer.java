...
public class HomeWorkMultiChatServer {

    // 대화명과 클라이언트의 소켓을 저장하기 위한 Map변수 선언
    private Map<String, Socket> clients;

    public HomeWorkMultiChatServer() {
    ...
    /**
     * Inner 클래스로 정의함. Inner 클래스로 정의하면 부모 클래스(Outer 클래스)의 멤버들을 직접 사용할 수 있다.
     */
    class ServerReceiver extends Thread {
        ...
        public void run() {
            try {
                ...
                while (dis != null) {
                    String msg = dis.readUTF(); // 수신한 메시지
                    ...
                    if (msg.startsWith("/w ")) { // 귓속말 메시지인 경우
                        ...
                        if (clients.containsKey(receiver)) { // 수신자가 존재하면 귓속말 메시지 전송
                            ...
                            receiverDos.writeUTF("[귓속말] From " + name + ": " + whisperMsg);
                        } else { // 수신자가 존재하지 않는 경우 에러 메시지 전송
                            DataOutputStream errorMsg = new DataOutputStream(socket.getOutputStream());
                            errorMsg.writeUTF("[에러] 수신자를 찾을 수 없습니다.");
                        }
                    } else {
                        // 일반 메시지 모든 클라이언트에게 전송
    ...
    public static void main(String[] args) {
        new HomeWorkMultiChatServer().serverStart();
    }
